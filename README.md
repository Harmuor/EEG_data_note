# Reference

1. Spapé, M. (2022). A psychologist’s guide to EEG: The electric study of the mind. SAGE Publications Ltd.

2. Jia, H. (n.d.). EEGLAB 操作手册（中文版）. School of Psychology, Central China Normal University.

3. Research group of brain and cognitive science (洪城课题组). [EEGLAB tutorial for mater program students (零基础脑电数据处理教程)](https://www.bilibili.com/video/BV1qj411w7wZ/?spm_id_from=333.1007.top_right_bar_window_history.content.click&vd_source=69f12cd3b0fc268b7bb22a55c4f209d7 'click it'). Jiangxi Normal university.

4. `MATLAB` edition: 2026a.

5. `EEGLAB` edition: 2026.0.0.

6.  `FieldTrip` edition: 20260812.

# Eprime-like

This is a PsychoPy file for running an oddball task to collect eeg data, following the tutorial by Spapé. Obviously I used fewer image material.

# Self_study

This is a dictionary containing MATLAB code and data files. But you need to download data [here](https://osf.io/4f5bn/files/w7d9f) and put them in right place according to the code content.

# `Learning_note.Rmd`

This is a `Rmarkdown` file that can be rendered into a `HTML` note. It's my study note.

# Note_image

This just is a dictionary contains images in the study note.
